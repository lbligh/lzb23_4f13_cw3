...
if __name__ == "__main__":
    ...
    top, top_axs = plt.subplots(1, 5, constrained_layout=True, figsize=(14, 3.2))

    for ax, alpha in enumerate([0.1, 50, 200, 1000]):
        plt.figure(top)
        ...
        top_axs[ax].set_title(rf"\textbf{{Top $\vert$ Alpha = {alpha}}}")
    ...
    top_axs[4].grid(alpha=0.4, zorder=0)

    top_axs[4].barh(bottom_words, bottom_frequencies, zorder=3)
    top_axs[4].set_title(rf"\textbf{{Bottom  $\vert$ Alpha = {alpha}}}")
    top_axs[4].set_ylabel(r"\textbf{Word}")
    top_axs[4].set_xlabel(r"\textbf{Probability}")

    # plt.suptitle(r"\textbf{Posterior Pred. Dist.}")
    plt.savefig(
        PLOT_DIR + "b_pred_histogram_new.png",
        dpi=500,
        format="png",
        bbox_inches="tight",
    )
